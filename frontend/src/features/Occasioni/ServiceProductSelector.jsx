...
export default function ServiceProductSelector({
  scope,
  services,
  products,
  selectedServiceIds,
  ...
  const toggleService = id => {
    const sid = String(id);
    const next = selectedServiceIds.includes(sid)
      ? selectedServiceIds.filter(x => x !== sid)
  ...
  const toggleProduct = id => {
    ...
    const next = selectedProductIds.includes(pid)
      ? selectedProductIds.filter(x => x !== pid)
    ...
    <div className="sps-wrap">
      ...
      {scope === "MIXED" && (
        <div className="sps-tab-bar">
          ...
          >
            Trattamenti{selectedServiceIds.length > 0 && (
              <span className="sps-count">{selectedServiceIds.length}</span>
            )}
          </button>
          <button
          ...
          >
            Prodotti{selectedProductIds.length > 0 && (
              <span className="sps-count">{selectedProductIds.length}</span>
            )}
          </button>
        </div>
      )}
      ...
      <div className="sps-search-wrap">
        ...
        />
        {search && (
          <button className="sps-clear" onClick={() => setSearch("")}>✕</button>
        )}
      </div>
      ...
      {(tab === "services" || !showProducts) && showServices && (
        <div className="sps-list">
          {filteredServices.length === 0 && (
            <p className="sps-empty">Nessun trattamento trovato</p>
          )}
          {filteredServices.map(s => {
            ...
            return (
              <label
                key={s.serviceId}
                className={`sps-item ${checked ? "sps-item--checked" : ""}`}
              >
                <input
                  type="checkbox"
                  checked={checked}
                ...
                {s.price && (
                  <span className="sps-item-price">
                    {Number(s.price).toLocaleString("it-IT", { style: "currency", currency: "EUR" })}
                  </span>
                )}
              </label>
            );
      ...
      {(tab === "products" || !showServices) && showProducts && (
        <div className="sps-list">
          {filteredProducts.length === 0 && (
            <p className="sps-empty">Nessun prodotto trovato</p>
          )}
