...
/**
 * WaitlistContent — inner content (state + form/success), shell-agnostic.
 * Reused as-is in both modes: inside UnifiedDrawer on desktop (WaitlistModal
 * wrapper below) and as an in-flow sub-view on the booking route.
 * Props: service { serviceId, title }, date: Date, slot { start, end }, onClose: fn
 */
export function WaitlistContent({ service, date, slot, onClose }) {
...
/**
 * Drawer che si apre quando la cliente clicca su uno slot occupato (desktop).
 * Props: show, onHide, service { serviceId, title }, date: Date, slot { start, end }
 */
export default function WaitlistModal({ show, onHide, service, date, slot }) {
  return (
    <UnifiedDrawer show={show} onHide={onHide} title="Lista d'attesa" size="sm" layout="side">
      <WaitlistContent service={service} date={date} slot={slot} onClose={onHide} />
    </UnifiedDrawer>
  );
}
